...
namespace EliteHospital.Data.Repository
{
    public class CMSAddLoyalityPointRepository
    {
        ...
        public tbl_PatientMaster Getpatientdetails(int Id)
        {
            try
            {
                tbl_PatientMaster trans = context.tbl_PatientMaster.Where(p => p.PM_Id == Id ).FirstOrDefault();
                return context.tbl_PatientMaster.Where(p => p.PM_ServerPatientId == trans.PM_ServerPatientId).FirstOrDefault();
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public void saveaddpoints(tbl_TransactionHistory TransactionHistory)
        {
            try
            {
                ...
                if (trans == null)
                {
                    ...
                    tbl_PatientMaster userdetails = context.tbl_PatientMaster.Where(p => p.PM_ServerPatientId == TransactionHistory.TH_PM_Id).FirstOrDefault();
                ...
                else
                {
                    ...
                    tbl_PatientMaster user = context.tbl_PatientMaster.Where(p => p.PM_ServerPatientId == TransactionHistory.TH_PM_Id).FirstOrDefault();
        ...
        public void saveservicemaster(tbl_ServiceMaster servicedetails)
        {
            ...
            else
            {
                ...
                updateresult.SM_Name_Arabic = servicedetails.SM_Name_Arabic;
                context.SaveChanges();
            }
        ...
        public tbl_TransactionHistory Checkpromocode(string promocode, int patientid)
        {
            try
            {
                return context.tbl_TransactionHistory.Where(p => p.TH_ServerPatientId == patientid && p.TH_Promocode == promocode && p.TH_Status != "RedeemApproval").FirstOrDefault();
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public void Updateredeempoints(tbl_TransactionHistory TransactionHistory)
        {
            try
            {
                ...
                trandetails.TH_CreatedBy = TransactionHistory.TH_CreatedBy;
                context.SaveChanges();
        ...
        public void SavePatientDetails(tbl_PatientMaster PatientDetails)
        {
            try
            {
                context.tbl_PatientMaster.Add(PatientDetails);
                context.SaveChanges();               
            }
        ...
        public tbl_TransactionHistory transactiondetails(int patientid)
        {
            try
            {
                return context.tbl_TransactionHistory.Where(p => p.TH_PM_Id == patientid && p.TH_Status=="RedeemPending").FirstOrDefault();
            }
            catch (Exception ex)
            {
                throw ex;
